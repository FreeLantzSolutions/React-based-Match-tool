...
//
//BOOL  CMatchToolDlg::Match(cv::Mat matSource, cv::Mat matDestination)
//{
//	if (matSource.empty() || matDestination.empty())
//		return FALSE;
//	if ((matDestination.cols < matSource.cols && matDestination.rows > matSource.rows) || (matDestination.cols > matSource.cols && matDestination.rows < matSource.rows))
//		return FALSE;
//	if (matDestination.size().area() > matSource.size().area())
//		return FALSE;
//	if (!m_TemplData.bIsPatternLearned)
//		return FALSE;
//	UpdateData(1);
...
//	if (m_ckBitwiseNot.GetCheck())
//	{
//		Mat matNewSrc = 255 - matSource;
//		buildPyramid(matNewSrc, vecMatSrcPyr, iTopLayer);
//		imshow("1", matNewSrc);
//		moveWindow("1", 0, 0);
//	}
//	else
//		buildPyramid(matSource, vecMatSrcPyr, iTopLayer);
//
//	s_TemplData* pTemplData = &m_TemplData;
//
//	//第一階段以最頂層找出大致角度與ROI
